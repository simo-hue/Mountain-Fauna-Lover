...
- [2026-06-25]: SEO + GEO Optimization Pass (Italian-anchored entity)
  - *Details*: Maximised search and generative-engine (AI answer engine) visibility **without changing any existing page copy**. Strategy: make the brand a machine-perfect, unambiguous *entity* anchored in Italian and in the target geography (Val di Rabbi, Parco Nazionale dello Stelvio, Trentino-Alto Adige), since the audience, videos and target queries are Italian. Scope agreed with owner: invisible technical/schema/metadata work plus additive FAQ sections and breadcrumbs only. Domain stays on `vercel.app` for now (accepted authority ceiling).
  - *Tech Notes — Structured data (JSON-LD):* New centralised builders in `src/lib/schema.ts` with stable `@id`s emitting a self-describing graph on every page: `WebSite`, `Organization`+`Brand`, `Person`, and `Place`/`Park`/`AdministrativeArea` nodes with `GeoCoordinates` for Val di Rabbi + Stelvio. Added `ProfilePage` (founder), `BreadcrumbList`, `FAQPage`, and `VideoObject` + `ItemList` for all 9 videos (`inLanguage: it-IT`, `creator`→Person, `contentLocation`→Place). New reusable `src/components/seo/JsonLd.tsx`; rewrote `src/components/seo/StructuredData.tsx`; new `src/lib/page-seo.ts` (`buildPageSeo`) to assemble each page's graph + breadcrumb trail from one translations read. Added geo/entity data + `places`/`topics`/`areaServed` to `src/config/site.ts`.
  - *Tech Notes — Additive UI:* New `src/components/seo/FaqSection.tsx` (native `<details>`, zero JS, crawlable) on home/digiscoping/collaboration/founder; new `src/components/seo/Breadcrumbs.tsx` rendered via a new optional `breadcrumb` slot in `PageHero`. Bilingual FAQ + breadcrumb copy added to `src/messages/{en,it}.json` (`faq.*`, `breadcrumb.home`), all derived strictly from facts already on the site.
  - *Tech Notes — Metadata & crawl:* `createPageMetadata` now emits `hreflang="x-default"`→`/it` and per-locale `og:locale` + `alternateLocale`. `src/app/robots.ts` explicitly welcomes AI crawlers (GPTBot, OAI-SearchBot, ChatGPT-User, ClaudeBot, PerplexityBot, Google-Extended, Applebot-Extended, CCBot, …). New `src/app/llms.txt/route.ts` (force-static) serving a curated Italian-anchored entity map at `/llms.txt`. `src/app/sitemap.ts` replaced per-build `new Date()` with a stable revision constant and added `x-default`. The chrome-less `/link` duplicate is now `noindex` + canonical→`/links`. `src/config/videos.ts` gained `uploadDate`/`duration` fields (null until owner fills).
  - *Tech Notes — Core Web Vitals (look preserved):* Added `preconnect`/`dns-prefetch` to `i.ytimg.com` and a hero LCP-image `preload` in `src/app/layout.tsx`; video thumbnails got descriptive `alt` (image SEO; button `aria-label` still wins for screen readers) and `priority` on the first card.
  - *Verification:* `npm run typecheck` clean, `npm run lint` clean, `npm run build` green (28 pages). Runtime-checked on `next start`: home emits the full entity graph incl. 2× `GeoCoordinates` + `FAQPage`; `/it/videos` emits 9× `VideoObject` + `ItemList` + `BreadcrumbList`; `/it/founder` emits `ProfilePage` + `FAQPage`; `/it/link` returns `noindex, follow`; `/llms.txt`, `/robots.txt` (AI bots), `/sitemap.xml` (`x-default`), per-locale `og:locale` and `hreflang` (en/it/x-default) all confirmed.
  - *Owner follow-ups (see `TO_SIMO_DO.md`):* Google + Bing Search Console verification (needs codes to add to site), social-name consistency + `sameAs` (YouTube shows "Mountain & Fauna Lover" vs brand "Mountain Fauna Lover"), Wikidata item, YouTube↔site binding, real video `uploadDate`/`duration`, and fix duplicate YouTube ID on `video-01`/`video-04`.

- [2026-06-25]: Navigation & Interaction Performance Pass (UX-preserving)
  - *Details*: Eliminated the perceived heaviness/lag on the two symptoms the owner reported — (a) clicking between pages and (b) moving the mouse (worst over the home-page header) — **without changing the visible cinematic experience**. Every change is a "same look, lighter engine" refactor: the per-frame GPU/CPU cost was removed while the design, motion, and layout were preserved. Decision scope agreed with owner via a grilling session before implementation.
  - *Tech Notes — Page transitions:* `src/app/[locale]/template.tsx` no longer animates `filter: blur(6px)→blur(0px)` across the whole incoming page (a full-page per-frame re-rasterisation on every navigation). It now runs a GPU-cheap opacity + 8px-rise fade at 0.3s and skips the animation entirely under `prefers-reduced-motion` (via `useReducedMotion`). Re-enabled Next.js `Link` prefetch on the primary navigation surfaces — `src/components/layout/Navbar.tsx` (logo + desktop + mobile nav) and `src/components/ui/ArrowLink.tsx` (in-content CTAs) — so destination payloads load ahead of the click instead of on it. Legal-only links (Footer `/privacy` `/cookies`, cookie banner, contact form) were intentionally left non-prefetching to avoid wasted requests for rarely-visited pages.
  - *Tech Notes — Cursor:* `src/components/cinematic/CustomCursor.tsx` dropped the invisible `backdrop-blur-[1px]` on the moving cursor element (it forced a per-frame backdrop snapshot on every page) and moved the `[data-cursor]` detection + state writes inside the rAF callback, updating React state only on an actual change. `mix-blend-difference`, spring sizing, and all cursor modes/icons are unchanged.
  - *Tech Notes — Navbar glass:* the sticky bar in `src/components/layout/Navbar.tsx` swapped `bg-black/45 + backdrop-blur-xl` for a solid translucent `bg-alpine-black/80` (no backdrop-filter). On the dark theme this is visually near-identical but removes the constant per-frame backdrop re-snapshot/blur sitting directly under the cursor at the top of the page.
  - *Tech Notes — Fog:* `src/components/cinematic/FogLayer.tsx` replaced the two `blur(90px)`/`blur(110px)` white blobs with soft `radial-gradient` blobs at the same opacities and the same drift animation. Removes the large blur textures (and their re-rasterisation under the cursor) on the home hero and on every subpage header (`PageHero`), plus `IdentityStatement`, `CollaborationPreview`, and `LogoPortalIntro`.
  - *Tech Notes — Observation lens:* `src/components/cinematic/ObservationLens.tsx` previously revealed the scene by animating `background-position` on a `filter`ed 256px circle — a CPU repaint on every mouse-move frame over the entire hero, the dominant cause of the home-header cursor lag. It now moves an inner background layer by the identical offset using a GPU `transform: translate3d()`, so the magnifier effect is pixel-identical but composited instead of repainted. The `contrast/brightness/saturate` filter, crosshairs, and "60× / REC" overlay are unchanged.
  - *Verification:* `npm run typecheck` clean, `npm run lint` clean, `npm run build` green (28 static pages). No new dependencies, environment variables, or assets; no owner action required.

- [2026-06-25]: Custom Cursor Personalization — Deer Antlers
  - *Details*: Replaced the default crosshair glyph of the bespoke desktop cursor with a custom deer-antler ("rack") mark drawn in the site's thin-line wireframe language, per owner request. Scope decided via a grilling session: antlers are the **resting** cursor only; the context-aware modes (▶ over videos, scope over the hero magnifier/digiscoping, magnetic dot over links/CTAs, technical/bike on gear) are preserved as interaction signals. At rest the antlers stand alone (the scope ring + faint fill fade to transparent); hovering anything interactive fades the ring back in around the contextual glyph. Static follow — no rotation/idle animation and no added per-move cost, so the prior performance pass stays intact. Touch + reduced-motion users keep the native cursor (gating unchanged).
  - *Tech Notes*: `src/components/cinematic/CustomCursor.tsx` only. Added a module-scope `Antlers` inline SVG (`viewBox="0 0 24 24"`, `stroke="currentColor"`, `strokeWidth` 1.7, round caps/joins; symmetric beams + two outward tines + a forked crown per side) so it inherits the cursor's `text-white` + `mix-blend-difference` and stays visible over any background. `default` mode now renders `<Antlers className="size-7" />` (28px) instead of `<Crosshair className="size-3" />` (Crosshair is still used by the `technical` mode). The ring `m.div` now animates `borderColor`/`backgroundColor` to transparent when not over an interactive target (resting = antlers only) and back to the visible scope ring + faint fill when active; resting box sized 30px. No new dependencies/assets/env. Verified `npm run typecheck`, `npm run lint`, `npm run build` (green, 28 static pages). A standalone shape-preview card was generated at `scratchpad/antler-cursor-preview.html` for quick visual review.

- [2026-06-26]: Cursor Antlers — Revised to a Detailed Filled Silhouette
  - *Details*: Superseded the first thin-line antler glyph (which read as crude at cursor size) with a detailed, realistic **filled-silhouette** deer rack, per owner feedback ("detailed and realistic"). The cursor *system* is unchanged — antlers remain the resting glyph only; the ▶ / scope / magnetic-dot context icons and the hover scope-ring are preserved; static follow; touch + reduced-motion users still get the native cursor. Only the antler artwork and its size changed.
  - *Tech Notes*: Rebuilt the `Antlers` SVG in `src/components/cinematic/CustomCursor.tsx` as a filled silhouette — tapered, round-joined beams + sharp triangular tine tips that merge into one organic shape — still using `currentColor` (keeps white + `mix-blend-difference`). Shipped the **"elegant"** red-deer variant (5 points/side; `viewBox="-39.25 -50.25 78.5 54.95"`, ≈1.43:1) rendered at `h-7 w-10` (40×28). Chosen over the wider **"monarch"** (1.97:1 — too flat at cursor scale) and **"reddeer"** (1.76:1) for cursor proportions + small-size legibility. Geometry is produced procedurally by `scratchpad/gen-antlers.js` (parametric beams/tines + auto bounding-box). Verified by rendering both the candidate sheet and the integrated cursor to PNG via headless Chrome and visually inspecting at 28–44px over near-black / charcoal / mist / ice / gradient / white — `mix-blend-difference` keeps it legible on all (it turns a warm antler-brown over the ice accent). `npm run typecheck`, `npm run lint`, `npm run build` all green (28 static pages). No new dependencies, assets, or env.

- [2026-06-26]: Cursor Antlers — Upgraded to a Realistic Red-Deer Rack
  - *Details*: Per owner feedback ("add more realism"), replaced the stylized "elegant" line/triangle silhouette with a realistic red-deer **"royal"** rack (6 points/side). The cursor *system* is unchanged — antlers are the resting glyph only; the ▶ / scope / dot context icons and hover scope-ring are preserved; static follow; touch + reduced-motion users keep the native cursor. Only the antler artwork (and a slight size bump) changed.
  - *Tech Notes*: New `src/components/cinematic/AntlerMark.tsx` holds the detailed art (isolated so `CustomCursor.tsx` stays readable). `CustomCursor` imports it and renders `<AntlerMark className="h-8 w-11" />` (≈44×32) as the `default` glyph. The rack is a filled silhouette built from variable-width **curved** beam/tine "ribbons" (centerline offset by ±width/2 along its normals) with sharp tapered tips, a knobby burr/coronet at the base, pearling bumps along the lower beam, and subtle deterministic left/right asymmetry — all `currentColor` (keeps white + `mix-blend-difference`, visible on any background). Geometry generated by `scratchpad/gen-antlers2.js` (the "royal" variant; "stag" = a leaner 5-point alternative). Verified by rendering both the candidate sheet and the integrated cursor (44×32, mix-blend) to PNG via headless Chrome and inspecting at 30–48px over near-black / charcoal / mist / ice / gradient / white — legible on all (warm antler-brown over the ice accent). `npm run typecheck`, `npm run lint`, `npm run build` all green (28 static pages). No new dependencies, env, or public assets (art is inline SVG).
